...
def show_random():
    ...
    # ми запускатимемо функцію, коли вікно вже є. Тож показуємо:
...
def edit_question(index):
    ...
    #  index - это об'єкт класу QModelIndex, див. потрібні методи нижче
    if index.isValid():
        i = index.row()
...
def connects():
    ...
    btn_add.clicked.connect(add_form) # споєднали натискання кнопки "нове питання" з функцією додавання
...
# а не при підключенні як модуль.
testlist()
set_card()
set_main()
connects()
